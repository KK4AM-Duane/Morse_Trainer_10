...
static String buildKochCharsJson() {
  ...
  for (int i = 0; i < count; i++) {
    ...
    else if (c == '>')  display = "BK";
    else if (c == '<')  display = "AS";
    else if (c == '$')  display = "KN";
...
void wsBroadcastStatus() {
    ...
    if (getWiFiOpMode() == WiFiOpMode::STA) {
        extern String getSTAIPAddress();
        extern bool   isSTAReady();
        wifiIP = isSTAReady() ? getSTAIPAddress() : "0.0.0.0";
    }
    else {
        wifiIP = "192.168.4.1";
    }
    ...
    String json = "{\"type\":\"status\""
        ...
        ",\"wifi_mode\":\"" + String(getWiFiOpModeName()) + "\""
        ",\"wifi_ip\":\"" + wifiIP + "\""
        "}";
    ws.textAll(json);
}
...
static void handleIncomingMessage(AsyncWebSocketClient *client, const String &msg) {
  ...
  if (type == "send") {
    ...
    if (!en && isEchoActive()) {
      // Buzzer locked on during echo training — re-broadcast current state
...
void wsBroadcastKeyerTx(bool on) {
  if (ws.count() == 0) return;
  ws.textAll(on ? "{\"type\":\"keyer_tx\",\"on\":true}"
                : "{\"type\":\"keyer_tx\",\"on\":false}");
}

void wsBroadcastKeyerPlay(const String &pattern, unsigned int ditMs) {
  if (ws.count() == 0) return;
  String json = "{\"type\":\"keyer_play\",\"pattern\":\"";
  json += pattern;
  json += "\",\"ditMs\":";
  json += String(ditMs);
  json += "}";
