...
// Keyer phone mute button
var elBtnKeyerMutePhone = null;
...
function cacheDom() {
    ...
    elBtnMutePhone = document.getElementById('btn-mute-phone');
    ...
    // Keyer phone mute button
    elBtnKeyerMutePhone = document.getElementById('btn-keyer-mute-phone');
}
...
function buildCharGrid() {
    ...
    if (activeKochCharacters.length === 0) {
        buildCharGridFallback();
        return;
    }

    var sorted = activeKochCharacters.slice().sort(function (a, b) {
...
function updateBuzzerButton() {
    ...
    if (echoActive) {
        // Show locked-on indicator during echo training
        elBtnBuzzer.textContent = '🔒 Buzzer';
        elBtnBuzzer.classList.remove('muted');
        elBtnBuzzer.classList.add('locked');
        elBtnBuzzer.disabled = true;
    } else {
        var label = buzzerEnabled ? '🔔 Buzzer' : '🔕 Muted';
        elBtnBuzzer.textContent = label;
        elBtnBuzzer.classList.remove('locked');
        if (muted) { elBtnBuzzer.classList.add('muted'); } else { elBtnBuzzer.classList.remove('muted'); }
        elBtnBuzzer.disabled = false;
    }

    if (elKeyerBuzzer) {
        if (echoActive) {
            elKeyerBuzzer.textContent = '🔒 Buzzer';
            elKeyerBuzzer.classList.remove('muted');
            elKeyerBuzzer.classList.add('locked');
        } else {
            var keyerLabel = buzzerEnabled ? '🔔 Buzzer' : '🔕 Muted';
            elKeyerBuzzer.textContent = keyerLabel;
            elKeyerBuzzer.classList.remove('locked');
            if (muted) { elKeyerBuzzer.classList.add('muted'); } else { elKeyerBuzzer.classList.remove('muted'); }
        }
    }
}

function cmdPhoneMuteToggle() {
    phoneMuted = !phoneMuted;
    if (initAudio()) {
        gainNode.gain.value = phoneMuted ? 0 : document.getElementById('vol-slider').value / 100;
    }
    updatePhoneMuteButton();
}

function cmdKeyerPhoneMuteToggle() {
    phoneMuted = !phoneMuted;
    if (initAudio()) {
        var vol = document.getElementById('keyer-vol-slider').value;
        gainNode.gain.value = phoneMuted ? 0 : vol / 100;
        // Sync the trainer vol slider
        var trainerSlider = document.getElementById('vol-slider');
        if (trainerSlider) trainerSlider.value = vol;
    }
    updatePhoneMuteButton();
}

function updatePhoneMuteButton() {
    if (!elBtnMutePhone) return;
    if (phoneMuted) {
        elBtnMutePhone.textContent = '📵 Muted';
        elBtnMutePhone.classList.add('muted');
    } else {
        elBtnMutePhone.textContent = '📱 Phone';
        elBtnMutePhone.classList.remove('muted');
    }
    // Sync keyer overlay phone mute button
    if (elBtnKeyerMutePhone) {
        if (phoneMuted) {
            elBtnKeyerMutePhone.textContent = '📵 Muted';
            elBtnKeyerMutePhone.classList.add('muted');
        } else {
            elBtnKeyerMutePhone.textContent = '📱 Phone';
            elBtnKeyerMutePhone.classList.remove('muted');
        }
    }
}
...
function testTone() {
    unlockAudio();
    // ESP32 plays on buzzer (respects on/off) and sends audio back to phone
    wsSend({ type: 'send', message: 'K' });
}
...
function sortCharGridAlphabetical() {
    if (activeKochCharacters.length === 0) return;

    activeKochCharacters.sort(function (a, b) {
        return charSortKey(a) - charSortKey(b);
    });
    buildCharGrid();
}
...
function ensureKeyerToneNode() {
    ...
    if (!keyerKeepAliveOsc) {
        ...
        keyerKeepAliveGain.gain.value = 0;
...
function buildKeyerSeq(pattern, ditMs) {
    ...
    for (var i = 0; i < pattern.length; i++) {
        ...
        if (el === '.' || el === '-') {
            ...
            seq.push({ tone: false, ms: ditMs * 3 });
        } else if (el === '|') {
            seq.push({ tone: false, ms: ditMs * 7 });
        }
    }
    return seq;
}