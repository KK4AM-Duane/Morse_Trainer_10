...
function cmdFileSave() {
  ...
  fetch('/api/practice', {
    ...
    .then(function () {
      elFileEditorStatus.textContent = 'Saved (' + text.length + ' bytes)';
      elFileEditorStatus.className = 'file-editor-status saved';
      logEvent('Practice file saved — ' + text.length + ' bytes');
    })
    .catch(function (err) {
...
function sortCharGridAlphabetical() {
  ...
  // Separate into letters, digits, punctuation, and prosigns for clean grouping
...
function sortKey(entry) {
  ...
  // Punctuation by char code: 36+
  if (d.length === 1) return 36 + c;
  // Prosigns (AR, BT, SK etc.): sort after everything else, alphabetically
  return 200 + c;
}

// Keyer mode variables
var keyerMode       = false;    // true when ESP32 is in CW Keyer mode
var keyerMemories   = ['', '', '', '', '', ''];  // M1–M6 contents
var keyerProgramSlot = -1;      // slot being programmed (-1 = none)
var keyerLongPressTimer = null;
var keyerLongPressSlot  = -1;
var keyerKeyTypeLPTimer  = null;
var keyerKeyTypeLPTarget = null;
...
function updateKeyerOverlay() {
  ...
  if (keyerMode) {
    ...
    // Request memory contents from ESP32
    wsSend({ type: 'keyer_mem_get' });
    updateKeyerKeyTypeButtons();
  } else {
    elKeyerOverlay.classList.add('hidden');
    // Cancel any open programming
    if (keyerProgramSlot >= 0) cmdKeyerMemCancel();
    cancelKeyerKeyTypeLongPress();
  }
}
...
function updateKeyerMemButtons() {
  for (var i = 0; i < 6; i++) {
    ...
    if (!btn) continue;
    btn.classList.remove('unprogrammed', 'programmed', 'playing', 'programming');
...
// Long-press detection: pointerdown starts a 3s timer, pointerup cancels
function setupKeyerLongPress() {
  for (var i = 0; i < 6; i++) {
    (function (slot) {
      var btn = elKeyerMemBtns[slot];
      if (!btn) return;

      btn.addEventListener('pointerdown', function (e) {
        e.preventDefault();
        keyerLongPressSlot = slot;
        keyerLongPressTimer = setTimeout(function () {
          keyerLongPressTimer = null;
          enterKeyerProgramMode(slot);
        }, 3000);
      });

      btn.addEventListener('pointerup', function () {
        if (keyerLongPressTimer) {
          clearTimeout(keyerLongPressTimer);
          keyerLongPressTimer = null;
        }
        keyerLongPressSlot = -1;
      });
  ...
  // Key type buttons — 3s long-press to change
...
function setupKeyerKeyTypeLongPress(btn, targetType) {
  ...
  btn.addEventListener('pointerdown', function (e) {
    e.preventDefault();
    // Already this type — nothing to do
    if (keyType === targetType) return;

    keyerKeyTypeLPTarget = targetType;
    // Visual feedback: show "confirming" animation while holding
    btn.classList.add('confirming');

    keyerKeyTypeLPTimer = setTimeout(function () {
      keyerKeyTypeLPTimer = null;
      keyerKeyTypeLPTarget = null;
      btn.classList.remove('confirming');

      // Send the change — NVS save happens on ESP32 side in setKeyType()
...
function cancelKeyerKeyTypeLongPress() {
  if (keyerKeyTypeLPTimer) {
    clearTimeout(keyerKeyTypeLPTimer);
    keyerKeyTypeLPTimer = null;
  }
  keyerKeyTypeLPTarget = null;
  // Remove confirming animation from both buttons
...
function cmdKeyerMemPlay(slot) {
  // If we're in programming mode, ignore short presses
  if (keyerProgramSlot >= 0) return;
  // Only play programmed memories
  if (!keyerMemories[slot] || keyerMemories[slot].length === 0) return;
