...
static void pollStraightKeyTx() {
    ...
    if ((millis() - txDebounceMs) > KEY_DEBOUNCE_MS) {
        if (reading != txKeyState) {
            ...
            if (txKeyState == LOW) {
                txOn();
                wsBroadcastKeyerTx(true);
            }
            else {
                txOff();
                wsBroadcastKeyerTx(false);
            }
        }
    }
...
static String buildKeyerPattern(const String& text) {
    String processed = preprocessProsigns(text);
    String pattern;
...
void setKeyerMemory(int slot, const String& text) {
    ...
    if (isStorageReady()) {
        char key[12];
        snprintf(key, sizeof(key), "keyMem%d", slot);
        getPrefs().putString(key, upper);
    }
    Serial.printf(">>> Keyer M%d saved: \"%s\" (%d chars)\n",
        slot + 1, upper.substring(0, 40).c_str(), upper.length());
}
